...
if file:
    ...
    # Gráfico de comparación Resultados vs Metas (tipo columnas agrupadas)
    st.subheader("📈 Comparación de Resultados y Metas")
    df_comparacion = df_filtrado.melt(
        id_vars=["Indicador", "Perspectiva"],
        value_vars=["Resultado", "Meta"],
        var_name="Tipo",
        value_name="Valor"
    )
    fig_comp = px.bar(
        df_comparacion, x="Indicador", y="Valor", color="Tipo", barmode="group",
        facet_col="Perspectiva", height=500
    )
